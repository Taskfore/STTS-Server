...
class ConversationResponseGenerator:
    ...
    def __init__(self, response_mode: str = "echo"):
        ...
        logger.info(
            f"ConversationResponseGenerator initialized with mode: {response_mode}"
        )
    ...
    def generate_response(
        ...
        logger.debug(
            f"Generated response for '{cleaned_input[:50]}...': '{response[:50]}...'"
        )
        return response
    ...
    def _generate_template_response(
        ...
        for category, templates in self.template_responses.items():
            if self._matches_category(input_lower, category):
                ...
                response = self._substitute_template_vars(
                    response_template, input_text, context
                )
                return response

        # No specific template match - use generic responses
        generic_responses = self.template_responses.get("generic", {}).get(
            "responses", []
        )
    ...
    def _substitute_template_vars(
        self, template: str, input_text: str, context: Optional[Dict[str, Any]] = None
    ) -> str:
    ...
    def set_response_mode(self, mode: str):
        ...
        if mode not in valid_modes:
            raise ValueError(
                f"Invalid response mode: {mode}. Valid modes: {valid_modes}"
            )
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        avg_input_length = (
            sum(len(h["input"]) for h in self.conversation_history) / total_exchanges
        )
