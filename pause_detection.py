...
class PauseDetector:
    ...
    def __init__(
        ...
        if not WEBRTC_AVAILABLE:
            raise ImportError(
                "webrtcvad library not available. Install with: pip install webrtcvad"
            )
        ...
        logger.info(
            f"PauseDetector initialized: aggressiveness={aggressiveness}, "
    ...
    def process_pcm_chunk(self, pcm_data: bytes) -> Dict[str, Any]:
        ...
        if self._chunk_count % 100 == 0:
            logger.info(
                f"PauseDetector processing chunk #{self._chunk_count}: {len(pcm_data)} bytes"
            )
        ...
        try:
            ...
            if self._chunk_count % 100 == 0:
                logger.info(
                    f"Converted PCM to numpy: {len(audio_np)} samples, dtype={audio_np.dtype}, range=[{audio_np.min()}, {audio_np.max()}]"
                )
        ...
        if self._chunk_count % 1000 == 0:
            logger.info(
                f"Frame processing: {processed_frames}/{total_frames} complete frames processed, frame_size={self.frame_size} samples ({self.frame_size * 2} bytes)"
            )
    ...
    def _process_frame(self, frame_bytes: bytes) -> list:
        ...
        try:
            ...
            if self._frame_count % 250 == 0:
                logger.info(
                    f"WebRTC VAD frame #{self._frame_count}: speech={is_speech}, frame_size={len(frame_bytes)} bytes, sample_rate={self.sample_rate}"
                )

        except Exception as e:
            logger.warning(
                f"WebRTC VAD error on frame #{getattr(self, '_frame_count', 0)}: {e}"
            )
            is_speech = False
        ...
        if self._frame_count % 250 == 0:
            logger.info(
                f"VAD counters: speech_frames={self.speech_frames}, silence_frames={self.silence_frames}, is_speaking={self.is_speaking}, min_speech={self.min_speech_frames}, min_pause={self.min_pause_frames}"
            )
        ...
        if not self.is_speaking and self.speech_frames >= self.min_speech_frames:
            ...
            logger.info(
                f"SPEECH START detected: speech_frames={self.speech_frames} >= min_speech_frames={self.min_speech_frames}"
            )
        ...
        # Speech end detection (pause)
            ...
            self.is_speaking = False
            speech_duration = (
                current_time - self.speech_start_time if self.speech_start_time else 0
            )
            events.append("speech_end")
            logger.info(
                f"SPEECH END detected: silence_frames={self.silence_frames} >= min_pause_frames={self.min_pause_frames}, speech_duration={speech_duration:.2f}s"
            )
...
class EnergyFallbackDetector:
    ...
    def __init__(
        ...
        logger.info(
            f"EnergyFallbackDetector initialized: threshold={silence_threshold_db}dB"
        )
