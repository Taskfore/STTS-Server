...
class ConversationProcessor:
    ...
    async def process_audio_chunk(self, audio_data: bytes) -> Optional[Dict[str, Any]]:
        ...
        if 'speech_start' in events:
            ...
            if self.conversation_state != "listening":
                logger.info(f"Speech started while in state '{self.conversation_state}' - resetting to listening")
        ...
        if 'speech_end' in events and not self.processing_transcription:
            logger.info("Speech pause detected - triggering transcription and response generation")
            ...
            if len(recent_audio) > 0:
                logger.info(f"Retrieved {len(recent_audio)} audio samples for transcription (last 5 seconds)")
                # Process transcription and response in background
                asyncio.create_task(self._process_speech_to_response(recent_audio))
            else:
                logger.warning("No audio samples available for transcription despite speech_end event")
        
        # Log pause detection status periodically
        if pause_result.get('is_speaking', False):
            logger.info(f"Voice activity detected: speaking={pause_result.get('is_speaking')}, confidence={pause_result.get('speech_confidence', 'N/A')}")
    ...
    async def _process_speech_to_response(self, audio_data: np.ndarray):
        ...
        logger.info(f"Starting speech-to-response processing for {len(audio_data)} audio samples")
        ...
        try:
            ...
            if not transcription_result or not transcription_result.text.strip():
                logger.info(f"STT transcription completed in {transcription_time:.2f}s but returned no text")
                return None
            
            transcribed_text = transcription_result.text.strip()
            logger.info(f"STT transcription completed in {transcription_time:.2f}s: '{transcribed_text}' (language: {transcription_result.language})")
            ...
            logger.info(f"Text response generated in {response_time:.3f}s: '{response_text}'")
            ...
            logger.info(f"Sending transcription to client: {len(transcription_result.segments)} segments, duration: {duration:.2f}s")
            ...
            if audio_response:
                audio_size_kb = len(audio_response['audio_data']) * 3 / 4 / 1024  # Rough base64 to bytes conversion
                logger.info(f"TTS synthesis completed in {tts_time:.2f}s: {audio_response['duration_ms']}ms audio, {audio_size_kb:.1f}KB")
            ...
            self.processing_transcription = False
            self.conversation_state = "listening"
            logger.info(f"Speech-to-response processing completed in {total_time:.2f}s total, returning to listening state")
    ...
    async def _transcribe_audio(self, audio_data: np.ndarray) -> Optional[TranscriptionResult]:
        ...
        try:
            logger.info(f"Submitting {len(audio_data)} audio samples to STT engine (language: {self.language or 'auto'})")
            ...
            if result:
                logger.info(f"STT engine returned result with {len(result.segments)} segments")
            else:
    ...
    async def _text_to_speech(self, text: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(f"Starting TTS synthesis for text: '{text}' using voice: {self.voice_path}")
            ...
            logger.info(f"TTS parameters: temp={temperature}, exag={exaggeration}, cfg={cfg_weight}, seed={seed}, speed={speed_factor}")
            ...
            logger.info(f"TTS engine generated audio tensor: shape={audio_tensor.shape}, sample_rate={sample_rate}")
            ...
            logger.info(f"Encoding audio to WAV format (target sample rate: {target_sample_rate})")
            ...
            logger.info(f"Audio base64 encoded: {len(audio_base64)} characters, duration: {duration_ms}ms")
...
async def websocket_conversation(
    ...
    logger.info(f"Retrieved STT engine from app state: model_loaded={stt_engine.model_loaded}")
            ...
            if data["type"] == "websocket.receive":
                if "bytes" in data:
                    ...
                    if audio_chunk_count % 50 == 0:
                        logger.info(f"Processing audio chunk #{audio_chunk_count}, size: {audio_size} bytes")
                    ...
                    try:
                        ...
                        logger.warning(f"Invalid JSON command received: {data['text']}, error: {e}")
                        await websocket.send_json({
                            "type": "error",
